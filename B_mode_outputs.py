...
def image_plot(B_mode_array, beam_spacing, axial_samples, num_beams, f_s, c):
    ...
    logging.info("Total Lateral Distance of image is " + str(lateral_distance) + "meters")
    logging.info("Total Depth of image is " + str(depth_distance) + "meters")
...
def image_save(image_filename, B_mode_array):
    try:
        ...
        logging.error("Attempted and failed to save using an unsupported filetype")
        raise ValueError
